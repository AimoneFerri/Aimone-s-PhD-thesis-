...
population_model <- '
  ...
  # Covariate effects
  Mental ~ 0.1*Sex + 0.1*Age + 0.1*BMI + 0.1*SES
  Cog    ~ 0.1*Sex + 0.1*Age + 0.1*BMI + 0.1*SES
  DietHealthy ~ 0.1*Sex + 0.1*Age + 0.1*BMI + 0.1*SES
...
fitted_model <- '
  ...
  # Covariates
  Mental ~ Sex + Age + BMI + SES
  Cog    ~ Sex + Age + BMI + SES
  DietHealthy ~ Sex + Age + BMI + SES
...
#save for OSF and to reload data if needed
